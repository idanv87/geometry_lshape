...
sample=grf(domain['interior_points'][:,0], 1, seed=0, sigma=sigma, mu=mu )
...
def deeponet(model, func):
    ...
    with torch.no_grad():
        ...
        pred2=model([y1, a1,a1*0,a1*0,angle1])
    return pred2.numpy()
...
def network(model, func, J, J_in, hint_init):
    ...
    for i in range(500):
        ...
        if (((k_it % J) in J_in) and (k_it > J_in[-1])):
            ...
            factor1=np.sqrt(2)*math.sqrt(0.1)/sigma
...
run_hints(func, J=J, J_in=[0], hint_init=True)
